...
export function Layout({ children }: { children: ReactNode }) {
  return (
    <div className="flex min-h-dvh flex-col">
      <a
        href="#main-content"
        className="fixed left-4 top-4 z-[60] -translate-y-24 rounded-btn bg-primary px-4 py-2.5 text-small font-medium text-ink-inverse transition-transform duration-200 focus:translate-y-0"
      >
        Skip to content
      </a>
      <ScrollManager />
      ...
      <Footer />
      <ConsentBanner />
      {/* Film grain over everything (fixed, non-interactive) */}
